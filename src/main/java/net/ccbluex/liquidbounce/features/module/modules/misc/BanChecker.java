                ...
                while (true) {
                    if (checkTimer.hasTimePassed(60000L)) {
                        try {
                            ...
                            if (jsonObject.get("success").getAsBoolean() && jsonObject.has("record")) {
                                ...
                                if (LiquidBounce.moduleManager.getModule(BanChecker.class).getState() && alertValue.get() && mc.thePlayer != null && (!serverCheckValue.get() || isOnHypixel()))
                                    if (STAFF_BAN_LAST_MIN > 0)
                                        LiquidBounce.hud.addNotification(new Notification("BanChecker","Staffs banned " + STAFF_BAN_LAST_MIN + " players in the last minute!", STAFF_BAN_LAST_MIN > 3 ? NotifyType.ERROR : NotifyType.WARNING, alertTimeValue.get() * 1000, 500));
                                    else
                                        LiquidBounce.hud.addNotification(new Notification("BanChecker","Staffs didn't ban any player in the last minute.", NotifyType.SUCCESS, alertTimeValue.get() * 1000, 500));
                            ...
                            if (LiquidBounce.moduleManager.getModule(BanChecker.class).getState() && alertValue.get() && mc.thePlayer != null && (!serverCheckValue.get() || isOnHypixel()))
                                LiquidBounce.hud.addNotification(new Notification("BanChecker","An error has occurred.", NotifyType.ERROR, 1000, 500));
                        }
                        checkTimer.reset();
                    }
                }
