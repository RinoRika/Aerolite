package net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer;

import net.ccbluex.liquidbounce.features.module.ModuleCategory;
import net.ccbluex.liquidbounce.launch.data.legacyui.ClickGUIModule;
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer.element.CategoryElement;
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer.element.SearchElement;
...
public class NewUi extends GuiScreen {
    ...
    private void drawFullSized(int mouseX, int mouseY, float partialTicks, Color accentColor) {
        ...
        if (ClientUtils.INSTANCE.mouseWithinBounds(mouseX, mouseY, this.width - 54F, 30F, this.width - 30F, 50F))
        ...
        if (Fonts.font40.getStringWidth(mc.thePlayer.getGameProfile().getName()) > 70)
            Fonts.font40.drawString(Fonts.font40.trimStringToWidth(mc.thePlayer.getGameProfile().getName(), 50) + "...", 100, 78 - Fonts.font40.FONT_HEIGHT + 15, -1);
        else
            Fonts.font40.drawString(mc.thePlayer.getGameProfile().getName(), 100, 78 - Fonts.font40.FONT_HEIGHT + 15, -1);
        ...
        for (CategoryElement ce : categoryElements) {
            ...
            if (ce.getFocused()) {
                startYAnim = AnimationUtils.animate(startY + 6F, startYAnim, (startYAnim - (startY + 5F) > 0 ? 0.65F : 0.55F) * RenderUtils.deltaTime * 0.025F);
                endYAnim = AnimationUtils.animate(startY + elementHeight - 6F, endYAnim, (endYAnim - (startY + elementHeight - 5F) < 0 ? 0.65F : 0.55F) * RenderUtils.deltaTime * 0.025F);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (!searchElement.isTyping()) for (CategoryElement ce : categoryElements) {
            ...
            if (ClientUtils.INSTANCE.mouseWithinBounds(mouseX, mouseY, 30F, startY, 230F, startY + elementHeight) && !searchElement.isTyping()) {
