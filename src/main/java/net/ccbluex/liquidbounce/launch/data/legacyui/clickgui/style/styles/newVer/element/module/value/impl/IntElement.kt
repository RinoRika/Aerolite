...
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer.element.components.Slider
...
class IntElement(val savedValue: IntegerValue): ValueElement<Int>(savedValue) {
    ...
    override fun onClick(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float) {
        ...
        if (ClientUtils.mouseWithinBounds(mouseX, mouseY, startPoint, y + 5F, endPoint, y + 15F))
            dragged = true
        if (ClientUtils.mouseWithinBounds(mouseX, mouseY, x + width - 5F - valueDisplay, y + 2F, x + width + 3F - valueDisplay, y + 18F))
            savedValue.set((savedValue.get() - 1).coerceIn(savedValue.minimum, savedValue.maximum))
        if (ClientUtils.mouseWithinBounds(mouseX, mouseY, x + width - 18F, y + 2F, x + width - 10F, y + 18F))
            savedValue.set((savedValue.get() + 1).coerceIn(savedValue.minimum, savedValue.maximum))
    }
