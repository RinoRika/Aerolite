package net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.flux.classic;
...
public class FluxClassic extends GuiScreen {
     ...
     */
    public FluxClassic() {
        this.clickGuiModule = (ClickGUIModule) LiquidBounce.moduleManager.getModule(ClickGUIModule.class);
    ...
    public void drawScreen(int mouseX,int mouseY,float partialTicks) {
        ...
        if(isHovered(windowsX + 390, windowsY + 245, windowsX + 400, windowsY + 250, mouseX,mouseY))
            Fonts.icon40.drawString("o", mouseX, mouseY, new Color(160,160,160).getRGB(), false);

        net.ccbluex.liquidbounce.utils.render.RenderUtils.drawImage(new ResourceLocation( "aerolite/clickgui/brush.png"), 9, height - 41, 32, 32);
        ...
        RenderUtils.drawRoundedRect3(windowsX, windowsY, windowsX + 100, windowsY + 280,2, new Color(255,255,255).getRGB(), 1);
        RenderUtils.drawRoundedRect3(windowsX + 100, windowsY, windowsX + 400, windowsY + 280,2, new Color(241,241,241).getRGB(), 2);
        for(int i = 0; i < ModuleCategory.values().length; i++) {
            ...
            Fonts.icon35.drawString(getCategoryIcon(categories[i]),windowsX + 10, windowsY + i * 25 + 47,categories[i] == currentCategory && (lastHeight > animationHeight ? animationHeight >= lastHeight : animationHeight <= lastHeight) ? -1 : new Color(160,160,160).getRGB());
            Fonts.gs35.drawString(categories[i].getDisplayName(),windowsX + 30, windowsY + i * 25 + 50, categories[i] == currentCategory && (lastHeight > animationHeight ? animationHeight >= lastHeight : animationHeight <= lastHeight) ? -1 : new Color(160,160,160).getRGB());
        }
        Fonts.icon35.drawString("q", windowsX + 5, windowsY + 10, new Color(70, 92 ,255).getRGB());
        Fonts.gs40.drawString("Aerolite", windowsX + 10 + Fonts.icon35.getStringWidth("q"), windowsY + 10, new Color(70, 92, 255).getRGB());
        RenderUtils.drawGradientSideways(windowsX + 100,windowsY, windowsX + 110, windowsY + 280, new Color(0, 0, 0, 70).getRGB(),new Color(241, 241, 241, 30).getRGB());
        ...
        for(final Module module : LiquidBounce.moduleManager.getModuleInCategory(this.currentCategory)) {
            if(!module.showSettings)
                ...
                for(int i = 0; i < module.getListValues().size(); ++i) {
                    ...
                    for(String mode : optionMode.getValues()) {
                        xPos += 14;
                        xPos += Fonts.gs35.getStringWidth(mode) + 10;
                ...
                for(int i = 0; i < module.getBooleanValues().size(); ++i) {
                    ...
                    if(module.getValues().size() == 1)
                        module.yPos1 += 10;
                    xPos += Fonts.gs35.getStringWidth(optionBoolean.getName()) + Fonts.icon35.getStringWidth("j") + 10;
            ...
            Fonts.gs35.drawString(module.getName(), startX + 20, startY + 14 + modulePosY, new Color(170,170,170).getRGB());
            Fonts.icon35.drawString(!module.showSettings ? "i" : "h", startX + 245, startY + 12 + modulePosY, module.showSettings || this.isHovered(startX + 245, startY + 12 + modulePosY, startX + 253, startY + 18 + modulePosY, mouseX, mouseY) ? new Color(86,147,245).getRGB() : new Color(160,160,160).getRGB());
            ...
            if(module.showSettings) {
                ...
                for(int i = 0; i < module.getNumberValues().size(); ++i) {
                    ...
                    if(option instanceof IntegerValue) {
                        ...
                        Fonts.gs35.drawString(optionInt.getName(), posX + 8, valuePosY + 5, new Color(160,160,160).getRGB());
                        ...
                        Fonts.gs35.drawString(optionInt.get().toString(), posX + 110, valuePosY + 5, new Color(160,160,160).getRGB());
                        ...
                        Fonts.gs35.drawString(optionDouble.getName(), posX + 8, valuePosY + 5, new Color(160,160,160).getRGB());
                        ...
                        Fonts.gs35.drawString(optionDouble.get().toString(), posX + 110, valuePosY + 5, new Color(160,160,160).getRGB());
                        if (this.isHovered(posX + 8, valuePosY + 14F, posX + 120, valuePosY + 19, mouseX, mouseY) && !mouseClicked && Mouse.isButtonDown(0))
                ...
                for(int i = 0; i < module.getListValues().size(); ++i) {
                    ...
                    if(module.getValues().size() == 1)
                        valuePosY += 10;
                    Fonts.gs35.drawString(optionMode.getName(), xPos, valuePosY + 5, new Color(98,154,247).getRGB());
                    valuePosY += 20;
                    moduleY += 20;
                    for(String mode : optionMode.getValues()) {
                        ...
                        xPos += 14;
                        Fonts.gs35.drawString(mode, xPos, valuePosY - 2, new Color(69,134,245).getRGB());
                        xPos += Fonts.gs35.getStringWidth(mode) + 10;
                        if(xPos > windowsX + 340 && optionMode.indexOf(mode) != optionMode.getValues().length - 1) {
                            valuePosY += 20;
                            xPos = startX + 8;
                ...
                for(int i = 0; i < module.getBooleanValues().size(); ++i) {
                    ...
                    Fonts.gs35.drawString(optionBoolean.getName(), xPos + Fonts.icon35.getStringWidth("j") + 4, valuePosY + 2, new Color(98,154,247).getRGB());
                    Fonts.icon35.drawString("j", xPos, valuePosY, optionBoolean.get() ? new Color(98,154,247).getRGB() : new Color(205,205,205).getRGB());
      ...
      /*  if(showConfig) {
            ...
            for (int index = 0;index < LiquidBounce.fileManager.profilesConfigs.size(); ++index) {
                ...
                Fonts.icon35.drawString("§妈x", width - 75, height - 3F - posY + yPos1, new Color(160,160,160).getRGB(), false);
                Fonts.gs35.drawString("§妈" + config.getFile().getName().split(".profile")[0], width - 75F + Fonts.icon35.getCharWidth('v') + 2, height - 4F - posY + yPos1, new Color(160,160,160).getRGB(), false);
            }
        } */
        Fonts.icon35.drawString("§妈v",width - 75, height - 22 - posY, new Color(160,160,160).getRGB(), false);
        Fonts.icon35.drawString("§妈u", width - 20, height - 22 - posY, new Color(160,160,160).getRGB(), false);
        Fonts.gs35.drawString("§妈#" + currentConfigName, width - 75F + Fonts.icon35.getCharWidth('v') + 2, height - 23F - posY, new Color(160,160,160).getRGB(), false);
