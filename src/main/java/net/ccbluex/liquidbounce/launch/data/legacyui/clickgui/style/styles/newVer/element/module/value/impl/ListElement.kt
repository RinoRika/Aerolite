...
class ListElement(val saveValue: ListValue): ValueElement<String>(saveValue) {
    ...
    companion object {
        val expanding = ResourceLocation("aerolite/clickgui/expand.png") }
    ...
    override fun onClick(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float) {
        if (isDisplayable() && ClientUtils.mouseWithinBounds(mouseX, mouseY, x, y + 2F, x + width, y + 18F))
            expansion = !expansion
        if (expansion) {
            var vertHeight = 0F
            for (subV in unusedValues) {
                if (ClientUtils.mouseWithinBounds(mouseX, mouseY, x + width - 14F - maxSubWidth, y + 18F + vertHeight, x + width - 10F, y + 34F + vertHeight)) {
                    value.set(subV)
                    expansion = false
                    break
                }
