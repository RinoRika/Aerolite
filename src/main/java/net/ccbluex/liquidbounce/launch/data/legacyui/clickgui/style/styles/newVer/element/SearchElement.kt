package net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer.element

import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer.ColorManager
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer.IconManager
...
class SearchElement(val xPos: Float, val yPos: Float, val width: Float, val height: Float) {
    ...
    fun handleMouseClick(mX: Int, mY: Int, mouseButton: Int, x: Float, y: Float, w: Float, h: Float, ces: List<CategoryElement>) {
        if (ClientUtils.mouseWithinBounds(mX, mY, x - 200F, y - 20F, x - 170F, y)) {
            searchBox.text = ""
            return
        }
        var mouseX = mX
