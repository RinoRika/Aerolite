...
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer.ColorManager
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer.element.components.ToggleSwitch
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer.element.module.value.ValueElement
...
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.newVer.extensions.animSmooth
...
class ModuleElement(val module: Module): MinecraftInstance() {
    ...
    companion object {
        protected val expandIcon = ResourceLocation("aerolite/clickgui/expand.png") }
    ...
    fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, height: Float, accentColor: Color): Float {
        ...
        if (ClientUtils.mouseWithinBounds(mouseX, mouseY, 
    ...
    fun handleClick(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, height: Float) {
        ...
        if (ClientUtils.mouseWithinBounds(mouseX, mouseY, 
            ...
            module.toggle()
        if (module.values.size > 0 && ClientUtils.mouseWithinBounds(mouseX, mouseY, x + width - 40F, y, x + width - 10F, y + height))
            expanded = !expanded
        if (expanded) {
            var startY = y + height
            for (ve in valueElements) {
