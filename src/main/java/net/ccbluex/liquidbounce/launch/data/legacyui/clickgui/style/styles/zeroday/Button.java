...
public class Button {
    ...
    public GameFontRenderer ListFont = Fonts.font35;
    public float x;
    public float y;
    ...
    public void render(int mouseX,int mouseY,Window window){
        animationY = y;
        if (module.getState()){
            this.parent.drawGradientRect(x,animationY-7,x+125,animationY+17,true, ColorUtils.INSTANCE.rainbow().getRGB(),ColorUtils.INSTANCE.rainbow(1).getRGB());
        }
        GlStateManager.resetColor();

        ListFont.drawString(module.getName(),x+5,animationY+1,-1);
        if (!module.getValues().isEmpty()) drawAndRotateArrow(x+110-0.2f,animationY  + 3 + 1f,8.0f,module.getArray());
